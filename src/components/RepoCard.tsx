...
export function RepoCard({ repo }: { repo: IRepo }) {
	...
	const [isFav, setIsFav] = useState(
		favorites.some((item) => item.url === repo.html_url)
	);
	...
	return (
		<div className="border py-3 px-5 rounded mb-2 shadow-md bg-gray-100 transition-all font-mono mx-auto max-w-[90vw]">
			<a href={repo.html_url} target="_blank" rel="noreferrer">
				...
				<h3 className="text-lg font-bold whitespace-pre-wrap break-words">
					{repo.name}
				</h3>
				<p className="text-sm">
					Forks: <span className="font-bold mr-2">{repo.forks}</span>
					Watchers:{' '}
					<span className="font-bold mr-2">{repo.watchers}</span>
					Language:{' '}
					<span className="font-bold">
						{switchIcon(repo.language)}
					</span>
				</p>
				...
				<p className="text-sm font-thin w-4/6 text-ellipsis overflow-hidden">
					{repo?.description}
				</p>
