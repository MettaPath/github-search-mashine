...
export function Profile() {
	const { updateDisplayName, updateStoreEmail, updateEmailVerified } =
		useActions();
	const userEmailVerified = auth.currentUser?.emailVerified;

	const { email, displayName, emailVerified } = useAppSelector(
	...
	const handleChangeEmail = async (e: React.FormEvent, newEmail: string) => {
		...
		if (newEmail === email) {
			setEmailError(
				'New email address should be different from current one'
			);
			return;
		}
		...
		if (user) {
			try {
				...
				setEmailSuccess(
					`Email has been updated and verification email has been sent on ${newEmail}`
				);
			} catch (error: any) {
				if (error.code === 'auth/requires-recent-login') {
					setEmailError(
						'Your credentials are too old. Please log in again.'
					);
					return;
				} else {
		...
		<div className="h-[100%] p-10 flex flex-col">
			<div className="flex flex-col justify-center lg:w-6/12 mx-auto mt-10 rounded shadow-md bg-gray-100 p-5">
				...
				<div>
					...
					<ImageUploadForm />
					<form
						className="flex flex-col md:w-9/12 rounded mb-10"
					...
					>
						...
						>
							...
							/>
							Name will be used in the account, you can always
							change it.
						</label>
						{!displayNameUpdated && (
						...
						<button
							className="w-fit shadow-md text-sm px-1 mb-1 h-[25px] bg-yellow-400 rounded md:hover:bg-sky-700 md:hover:text-white transition-all"
							type="submit"
						>
							Save changes
						</button>
				...
				<div>
					...
					<form
						className="flex flex-col md:w-9/12 rounded mb-10"
						onSubmit={(e) => handleChangeEmail(e, newEmail)}
					>
						<label
						...
						>
							Email address
							{emailVerified && (
								<span className="absolute left-24 top-0.5 text-xs text-green-600">
									Verified
								</span>
							)}
							{!emailVerified && (
								<span className="absolute left-24 top-0.5 text-xs text-red-500">
									Unverified
								</span>
							)}
							<input
							...
							/>
							The address is used to sign in to your account and
