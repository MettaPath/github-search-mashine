  ...
  return (
    ...
    <div className="w-screen h-[70px] shadow-md bg-gray-500 font-mono fixed z-30 mb-10">
      <nav className="container text-sm mx-auto flex justify-between items-center h-[70px] px-5 text-white">
        ...
        <Link to="/" className="font-mono text-lg mr-2 hidden md:block">GitHub Search</Link>
        <Link to="/"><GitHub/></Link>
        </div>
        <span className="lg:flex flex-row justify-center items-center hidden">
          <Link to="/" className="mr-3 md:hover:text-neutral-900 transition-all">Home</Link>
          {!isAuthenticated &&
          <div>
          ...
          {isAuthenticated &&
            <div className="flex flex-row items-center">
              <Link to="/profile" className="md:hover:text-neutral-900 rounded transition-all mr-3">Profile</Link>
              <button
            ...
            Sign out
              ...
              <div className="flex flex-row items-center border rounded p-1">
                <div className="flex flex-col text-xs">
                  <p className="font-thin mr-2">You sign in as:</p>
                  <p className="mr-2">{displayName ? displayName : email}</p>
                </div>
                <User />
                </div>
            </div>
